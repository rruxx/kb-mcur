...
pub fn sub_key_index(ch: char) -> Option<usize> {
    for row in 0..2 { for col in 0..4 {
        if SUBGRID_LABELS[row][col] == ch { return Some(row * 4 + col); }
    }}
    None
}

pub fn is_sub_key(ch: char) -> bool { sub_key_index(ch).is_some() }

// ── Levels 4-7  bisect keys (2×2) ──────────────────────────────────

pub const BISECT_LABELS: [[char; 2]; 2] = [
    ['e', 'r'],
    ['d', 'f'],
];
...
pub fn quad_key_index(ch: char) -> Option<usize> {
    for row in 0..2 { for col in 0..2 {
        if BISECT_LABELS[row][col] == ch { return Some(row * 2 + col); }
    }}
    None
}

pub fn is_quad_key(ch: char) -> bool { quad_key_index(ch).is_some() }
...
pub fn action_key(ch: char) -> Option<(u8, bool)> {
    for &(k, btn) in &TOGGLE_KEYS { if k == ch { return Some((btn, false)); } }
    for &(k, btn) in &CLICK_KEYS { if k == ch { return Some((btn, true)); } }
    None
}
