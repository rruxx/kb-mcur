...
impl Dispatch<WlRegistry, GlobalListContents> for WlrBackend { fn event(_: &mut Self, _: &WlRegistry, _: <WlRegistry as Proxy>::Event, _: &GlobalListContents, _: &Connection, _: &QueueHandle<Self>) {} }
...
impl Dispatch<ZwlrLayerSurfaceV1, ()> for WlrBackend {
    fn event(_: &mut Self, proxy: &ZwlrLayerSurfaceV1, event: <ZwlrLayerSurfaceV1 as Proxy>::Event, _: &(), _: &Connection, _: &QueueHandle<Self>) {
...
impl WlrBackend {
    pub fn connect() -> Result<Self> {
        ...
        let layer_shell: ZwlrLayerShellV1 = globals.bind(&qh, 1..=5, ()).context("zwlr_layer_shell_v1")?;
        let vptr_mgr: ZwlrVirtualPointerManagerV1 = globals.bind(&qh, 1..=2, ()).context("zwlr_virtual_pointer_manager_v1")?;
    ...
    pub fn add_window(&mut self, _x: i32, _y: i32, w: u16, h: u16) -> Result<usize> {
        ...
        self.windows.push(LayerWin { surface, w: w as i32, h: h as i32, pool_off: 0 });
    ...
    pub fn upload(&self, idx: usize, skia: &SkiaPixmap) -> Result<()> {
        ...
        for row in 0..win.h as usize {
            ...
            for i in (0..stride).step_by(4) {
                d[i] = s[i + 2]; d[i + 1] = s[i + 1]; d[i + 2] = s[i]; d[i + 3] = s[i + 3];
            }
        }
        let pool = self.shm_pool.as_ref().context("no shm pool")?;
        let eq = self.conn.new_event_queue::<WlrBackend>();
        let qh = eq.handle();
        let buf = pool.create_buffer(win.pool_off as i32, win.w, win.h, stride as i32, wl_shm::Format::Argb8888, &qh, ());
    ...
    pub fn pointer_warp(&self, x: i16, y: i16) -> Result<()> {
        ...
        let sx = self.monitors[0].3 as u32;
        let sy = self.monitors[0].4 as u32;
...
impl WlrBackend {
    fn ensure_shm_pool(&mut self) -> Result<()> {
        if self.shm_fd.is_some() { return Ok(()); }
...
fn shm_fd(size: u32) -> Result<OwnedFd> {
    ...
    let path = std::env::temp_dir().join(format!("kb-mcur-shm-{}", std::process::id()));
